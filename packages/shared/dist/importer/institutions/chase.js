...
import { GeneralTransaction, TransactionType, TransferTransaction } from '../../models/transaction.js';
...
export class ChaseCsvStatementImporter extends StatementImporter {
    ...
    async processTransaction(record) {
        ...
        if (transactionType === TransactionType.Transfer) {
            ...
            this.userId, amount, this.currency, date, description, CategoryType.Transfer, // categoryId
            [] // tagIds
            );
        }
        return new GeneralTransaction(this.accountId, this.userId, amount, this.currency, date, description, false, // isTaxDeductable
        false, // hasCapitalGains
        null, // merchant
        CategoryType.Other, [], // tagIds
        transactionType);
    }
}
/**
...
export class ChaseCreditCsvStatementImporter extends StatementImporter {
    ...
    async processTransaction(record) {
        ...
        if (transactionType === TransactionType.Transfer) {
            return new TransferTransaction(this.accountId, 'unknown_linked_tx', // We don't know the linked transaction yet
            this.userId, amount, this.currency, date, description, category, // categoryId
            [] // tagIds
            );
        }
        return new GeneralTransaction(this.accountId, this.userId, amount, this.currency, date, description, false, // isTaxDeductable
