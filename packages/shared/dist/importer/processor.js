...
export class TransactionProcessor {
    ...
    groupLines(lines) {
        if (!this.bankConfig || !this.bankConfig.startDelimiter)
            return lines;
        const results = [];
        let currentBlock = [];
        for (const line of lines) {
            if (this.bankConfig.startDelimiter.test(line)) {
                if (currentBlock.length > 0)
                    results.push(currentBlock.join(' ')); // Join with space for regex
    ...
    parseCsv(statementData) {
        ...
        for (const row of rows) {
            if (row.length === 0 || (row.length === 1 && !row[0]))
                continue;
            for (const rule of this.bankConfig.transactionTypes) {
                if (rule.identifier && !rule.identifier(row))
                    continue;
    ...
    mapTransaction(rule, sourceData, match, baseDate, raw) {
        const txn = {
